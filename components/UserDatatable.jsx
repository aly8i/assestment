  ...
  const requestSearch = (searchedVal) => {
    if(searchedVal!=""){
      const filteredRows = rows.filter((row) => {
        return row.name?.toLowerCase().includes(searchedVal.toLowerCase())||row._id.includes(searchedVal.toLowerCase())||row.phonenumber?.includes(searchedVal.toLowerCase());
      });
      ...
      renderCell: (params) => {
        return (
          <div className={styles.cellAction}>
            <Link href={`/customers/${params.row._id}`} passHref style={{ textDecoration: "none" }}>
